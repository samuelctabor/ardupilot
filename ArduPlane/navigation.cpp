    ...
    if (reached_target) {
        ...
        */
        ...
        if (terrain_status_ok &&
            fabsf(altitude_agl - altitudePlanner.get_terrain_alt_cm()*0.01) < 5) {
    ...
    if (now - fbwb_altitude.last_elev_check_us >= 100000) {
        ...
        if (g2.soaring_controller.is_active()) {
            if (g2.soaring_controller.get_throttle_suppressed()) {
                ...
                altitudePlanner.set_target_altitude_amsl_cm(100*plane.g2.soaring_controller.get_alt_cutoff() + 1000 + AP::ahrs().get_home().alt);
            }
        }
#endif
