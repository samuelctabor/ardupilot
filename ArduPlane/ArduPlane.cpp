...
void Plane::update_alt()
{
    ...
    if (auto_throttle_mode && !throttle_suppressed) {        
        ...
        float target_alt = altitudePlanner.get_relative_target_altitude_cm(lookahead, range_correction);
...
float Plane::tecs_hgt_afe(void)
{
    ...
    */
    ...
    if (flight_stage == AP_Vehicle::FixedWing::FLIGHT_LAND) {
        hgt_afe = altitudePlanner.get_height_above_target(current_loc, next_WP_loc);
