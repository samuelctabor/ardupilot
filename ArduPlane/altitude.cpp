    ...
    if (control_mode == &mode_guided && ((guided_state.target_alt_time_ms != 0) || guided_state.target_alt > -0.001 )) { // target_alt now defaults to -1, and _time_ms defaults to zero.
      ...
      if (landing.is_flaring()) {
       ...
       altitudePlanner.reset_offset_altitude();
        ...
        altitudePlanner.set_target_altitude_proportion(next_WP_loc, 1.0f-auto_state.wp_proportion, suppress_glideslope);
 ...
 */
void AP_AltitudePlanner::set_target_altitude_proportion(const Location &loc, float proportion, bool suppress_glideslope)
{
