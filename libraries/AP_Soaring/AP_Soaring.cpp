...
SoaringController::ThermalStatus SoaringController::check_cruise_criteria(Vector2f prev_wp, Vector2f next_wp)
{
    ActiveStatus status = active_state();

    if (status == ActiveStatus::SOARING_DISABLED) {
        _cruise_criteria_msg_last = ThermalStatus::DISABLED;
        return ThermalStatus::DISABLED;
    }

    ThermalStatus result = ThermalStatus::THERMAL_OK;
    const float alt = _vario.alt;

    if (_exit_commanded) {
        result = ThermalStatus::EXIT_COMMANDED;
    } else if (alt > alt_max) {
        result = ThermalStatus::ALT_TOO_HIGH;
        if (result != _cruise_criteria_msg_last) {
            gcs().send_text(MAV_SEVERITY_ALERT, "Reached upper alt = %dm", (int16_t)alt);
        }
    } else if (alt < alt_min) {
        result = ThermalStatus::ALT_TOO_LOW;
        if (result != _cruise_criteria_msg_last) {
        ...
        if (_thermalability < mcCreadyAlt) {
            result = ThermalStatus::THERMAL_WEAK;
            if (result != _cruise_criteria_msg_last) {
